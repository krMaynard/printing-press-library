# Naver Privacy Printed CLI Agent Guide

This directory is a generated `naver-privacy-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
naver-privacy-pp-cli doctor --json
naver-privacy-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
naver-privacy-pp-cli which "<capability>" --json
naver-privacy-pp-cli <command> --help
```
...
```bash
naver-privacy-pp-cli <command> --agent
```
...
```bash
naver-privacy-pp-cli <command> --help
naver-privacy-pp-cli <command> --dry-run --agent
```
