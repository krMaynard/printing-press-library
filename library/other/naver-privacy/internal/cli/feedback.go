...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "naver-privacy-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NAVER_PRIVACY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NAVER_PRIVACY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "naver-privacy-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/naver-privacy-pp-cli/feedback.jsonl.
When ` + "`NAVER_PRIVACY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NAVER_PRIVACY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  naver-privacy-pp-cli feedback list
  naver-privacy-pp-cli feedback list --limit 5
  naver-privacy-pp-cli feedback list --json`,
