...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://privacy.naver.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("NAVER_PRIVACY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "naver-privacy-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("NAVER_PRIVACY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
